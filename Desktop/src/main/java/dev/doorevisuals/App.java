...
import dev.doorevisuals.server.InvManagerFeature;
...
import dev.doorevisuals.tools.ItemSwapFeature;
...
import dev.doorevisuals.tools.PickupLoggerFeature;
...
public final class App {
    ...
    private synchronized void start() {
        if (!this.live) {
            LOG.info("Bootstrapping DooreVisuals {}", "3.32.0");
            BootDebugLog.write("H1", "App.java:start", "mod bootstrap reached", "{\"version\":\"3.32.0\"}");
            ModSounds.boot();
            this.aim = new Aim();
            this.aim.bind();
    ...
    private static void install(FeatureBus bus, DiscordFeature discord, GpsFeature gps, HitFxFeature hitFx, KillFxFeature killFx, HitSoundFeature hitSound) {
        ...
        bus.add(new ItemSwapFeature());
        bus.add(new ElytraSwapFeature());
        bus.add(new InvToolsFeature());
        bus.add(new AhHelperFeature());
        bus.add(new AutoInvestFeature());
        bus.add(new FunHelperFeature());
        bus.add(new EventsFeature());
        bus.add(new InvManagerFeature());
        bus.add(new CommandFixFeature());
        bus.add(new PvpSaveFeature());
        bus.add(new CooldownsFeature());
        bus.add(new PickupLoggerFeature());
        bus.add(new BoreHelperFeature());
        bus.add(new LockSlotFeature());
        bus.add(new ModelsFeature());
    }
}
